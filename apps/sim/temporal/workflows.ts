...
export async function executeWorkflowLoop(
  ...
  const starter = workflowDefinition.blocks.find(
    (b) =>
      b.metadata?.id === 'starter' || (triggerType === 'webhook' && b.config.params?.triggerMode)
  )
    ...
    if (pendingBlocks.length > 0) {
      ...
      await Promise.all(
        pendingBlocks.map(async (block) => {
          ...
          try {
            const result = await executeBlockActivity(block.id, workflowDefinition, currentContext)
  ...
  await completeLoggingSessionActivity(workflowId, executionId, triggerType, {
...
export async function runWebhookWorkflow(params: {
  payload: any
  executionId: string
}): Promise<any> {
